	...
	<div class="select__wrap">
		<select name="salon" class="selectric selectric_pdp">
			...
			foreach( pdp_get_salons() as $salon ) :
                if( carbon_get_post_meta( $salon->ID, 'display_in_forms' ) === 'yes' ) :
                    $city_terms = get_the_terms( $salon->ID, 'city' );
                    $city = array_pop( $city_terms )->name; ?>
                        <option value="<?=$salon->ID; ?>" <?=( isset( $_GET['salon_pricelist'] ) && $_GET['salon_pricelist'] == $salon->ID ) ? 'selected' : ''; ?>><?=$city . ', ' . $salon->post_title; ?></option>
            <?php
                endif;
            endforeach; ?>
		</select>
